        ...
        if self.__values[j] == 'R':
            if j < self.__size - 1:
                if self.__values[j + 1] == 0:
                    aux = self.__values[:]
                    aux[j], aux[j + 1] = 0, 'R'
                    nextC.append(Configuration(aux))  # one step forward for a red frog
            if j < self.__size - 2:
                if self.__values[j + 2] == 0:
                    aux = self.__values[:]
                    aux[j], aux[j + 2] = 0, 'R'
                    nextC.append(Configuration(aux))  # twp steps forward for a red frog
        elif self.__values[j] == 'B':
            if j > 0:
                if self.__values[j - 1] == 0:
                    aux = self.__values[:]
                    aux[j], aux[j - 1] = 0, 'B'
                    nextC.append(Configuration(aux))  # one step backward for a brown frog
            if j > 1:
                if self.__values[j - 2] == 0:
                    aux = self.__values[:]
                    aux[j], aux[j - 2] = 0, 'B'
                    nextC.append(Configuration(aux))  # two steps backward for a brown frog
        return nextC
